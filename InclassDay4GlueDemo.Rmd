---
title: "In Class GLUE with Distributed model"
...
Load the script and the data for the SL distributed model
...
Select a window of the data 2000 - 2005
```{r}
# Use data for 2000 - 2005
Data_in <- window(SL_distdata, start = "2000-01-01",
                  end ="2005-12-31")
```
...
Make a histogram of the performance measures
...
## Combine parameters space with performance
...
# now combine
Store_out_comb <- 
        cbind(Store_out_sort,parameterSpace_stacked)
```

Plot the subbasin parameters by r.squared
...
This creates a reduced set of simulations

## step 2 
Find the related parameterSpace
...
The problem is that we want to save the predicted timeseries, not just the performance
...
This gives three sets of data, with in columns the "good behaviour" predicted output for each station

## step 4 plot the GLUE

Now plot the GLUE results
...
plot_df %>%
      gather(key="Glue_results", 
             value="value", mean:pos_95ci) %>%
